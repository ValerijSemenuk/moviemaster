...
class _MovieDetailsPageState extends State<MovieDetailsPage> with SingleTickerProviderStateMixin {
  ...
  Widget _buildTrailerCard(VideoEntity video) {
    return Card(
  ...
  Widget _buildVideosTab(MovieDetailsEntity movie) {
    final youtubeVideos = movie.videos.where((video) => video.site == 'YouTube').toList();
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) => _buildTrailerCard(youtubeVideos[index]),
    );
  }

  Widget _buildBackdrop(MovieDetailsEntity movie) {
    return Stack(
      children: [
  ...
  Widget _buildOverviewTab(MovieDetailsEntity movie) {
  ...
  Widget _buildCastTab(MovieDetailsEntity movie) {
  ...
  Widget _buildReviewsTab(MovieDetailsEntity movie) {
  ...
  Widget _buildSimilarMoviesTab(MovieDetailsEntity movie) {
      ...
      return const Center(
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 12),
            Text('Схожих фільмів не знайдено'),
          ],
        ),
      );
    }
                ...
                headerSliverBuilder: (context, innerBoxIsScrolled) {
                  return [
                    SliverAppBar(
                      ...
                      floating: false,
                      pinned: true,
                      flexibleSpace: _buildBackdrop(movie),
                      leading: IconButton(
                        icon: Container(
